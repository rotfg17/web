...
$sql = $con->prepare("SELECT id, nombre, precio FROM productos WHERE id_categoria = 4 LIMIT :offset, :resultados_por_pagina");
...
$sql_count = $con->prepare("SELECT COUNT(*) AS total FROM productos WHERE id_categoria = 4");
...
<html lang="en">
    <head>
        ...
        <title>Ferre Seibo - Cerrajería	</title>
    </head>
<body>
   ...
   <img class="img-responsive vdk" src="img/cerrajeria.webp" alt="cerrajeria" width="100%" height="auto" >
    </div>
...
<section class="paginacion">
    <ul>
        ...
        for ($i = 1; $i <= $total_paginas; $i++) {
            // Agregar la clase "active" al enlace de la página actual
            $claseActiva = ($i == $pagina_actual) ? 'active' : '';
            echo "<li><a href='cerrajeria.php?pagina=$i' class='$claseActiva'>$i</a></li>";
        }
        ?>
    </ul>
</section>
