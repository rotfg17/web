/*ESTE CODIGO SIRVE PARA QUE CUANDO DES UN CLICK EN LA PANTALLA SE OCULTE EL CONTENIDO*/
$(document).ready(function () {
    $(".menu-icon1").click(function (event) {
        event.stopPropagation(); // Evita que el clic se propague al documento
        $(".menu1").toggle();
    });
    $(document).on("click", function (event) {
        if (!$(event.target).closest(".menu1").length && !$(event.target).closest(".menu-icon1").length) {
            $(".menu1").hide();
        }
    });    
});
...
/** ESTE CODIGO SIRVE PARA BUSCAR LOS PRODUCTOS */
