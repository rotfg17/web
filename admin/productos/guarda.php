...
//Declaracion de variables que se encuentran dentro de la base de datos
$nombre = $_POST['nombre'];
$marca = $_POST['marca'];
...
//Sirve para insertar la informacion que tenemos en la base de datos
$sql = "INSERT INTO productos (nombre, marca, descripcion, precio, descuento, stock, num_referencia, id_categoria, activo) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)";
$stm = $con->prepare($sql);
if($stm->execute([$nombre, $marca, $descripcion, $precio, $descuento, $stock, $referencia, $categoria])){
   $id = $con->lastInsertId();
